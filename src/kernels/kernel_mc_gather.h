...
class KernelMcGather {
 ...
 public:
  ...
  __aicore__ inline KernelMcGather(uint32_t idx_in_len, uint32_t tile_len)
   ...
   * of size s,
   *
   * index_tile[s-1] - index_tile[0] < 32768 is mandatory
   */
  ...
  __aicore__ inline void ProcessTile(uint32_t output_gm) {
    ...
    if (es_diff < value_tile_size_) {
      ...
      copy::CopyVecToGm(global_z_[output_gm], output_q_, tile_len_);
    } else {
      uint32_t i = 0;
      ...
      while (end - new_start >= value_tile_size_) {
        ...
        LocalTensor<uint32_t> gathered_idx_lt = FilterTile<uint32_t>(
            idx_lt, threshold_up, threshold_down, gathered_size);
        ...
        LocalTensor<DataType> sync_fetched_values =
            values_q_gather_.DeQue<DataType>();
        GatherWithOffset<uint32_t, DataType>(z_lt, gathered_idx_lt,
                                             sync_fetched_values, new_start,
                                             subtile_size);
        output_q_.EnQue<DataType>(z_lt);
      ...
      threshold_down = idx_lt.GetValue(gathered_count) - 1;
      gathered_idx_lt = FilterTile<uint32_t>(idx_lt, threshold_down,
                                             gathered_size, CMPMODE::GT);
      new_start = gathered_idx_lt.GetValue(0);
      subtile_size = static_cast<uint32_t>(gathered_size);
      copy::CopyGmToVec(values_q_gather_, global_values_[new_start],
                        remaining_tile);
   ...
   * The number of gathered elements is stored in the output parameter
   * @p gathered_size.
   *
   *
   ...
   * this value.
   * @param gathered_size A reference to a variable where the count of valid
   * (gathered) elements will be stored.
   * @param mode Ascend::CMPMODE for comparing threshold and tensor
   * @return LocalTensor<T> A tensor containing the indices of elements that
  ...
  __aicore__ inline LocalTensor<T> FilterTile(LocalTensor<T> idx_lt,
    ...
    uint32_t tile_len = idx_lt.GetSize();
   ...
   * values. The number of gathered elements is stored in the output parameter
   * @p gathered_size.
   *
   *
   * @tparam T The data type of the tensor elements.
  ...
  template <typename T>
  __aicore__ inline LocalTensor<T> FilterTile(LocalTensor<T> idx_lt,
                                              T threshold_up, T threshold_down,
                                              uint64_t &gathered_size) {
    uint32_t tile_len = idx_lt.GetSize();
  ...
  /**
   * @brief Gathers elements from an input tensor using an index tensor with
   ...
   * @param idx_lt The local tensor containing indexes that will be adjusted
   * and used for gathering.
...
__aicore__ inline void run_mc_gather(GM_ADDR values_in, GM_ADDR idx_in,
  ...
  if ASCEND_IS_AIV {
    KernelMcGather<float> op(idx_in_len, tile_len);
