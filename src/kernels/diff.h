...
class KernelDiff {
 ...
 public:
  ...
  __aicore__ inline void Init(GM_ADDR vec_in, GM_ADDR vec_out) {
    ...
    global_out_.SetGlobalBuffer((__gm__ OutputT *)vec_out + 1, vec_len_ - 1);
    global_out_first_elem_.SetGlobalBuffer((__gm__ OutputT *)vec_out, 1);

    pipe.InitBuffer(in_q_, BUFFER_NUM, (tile_len_ + 1) * sizeof(InputT));
 ...
 private:
  __aicore__ inline void ProcessTile(uint32_t progress, uint32_t num_elems_) {
    ...
    // Create index vector: 2,4,6,...
    ArithProgression<int32_t>(cols_int32_lt, static_cast<int32_t>(2),
                              static_cast<int32_t>(2), num_elems_ - 1);
    ...
    vec_out_lt.SetValue(num_elems_ - 1, static_cast<half>(delta));

    // Set manually the first element in GM
    if (GetBlockIdx() == 0 && progress == 0) {
      global_out_first_elem_(0) = vec_in_lt(0);
      DataCacheCleanAndInvalid<InputT, CacheLine::SINGLE_CACHE_LINE,
  ...
  GlobalTensor<OutputT> global_out_first_elem_;
...
__aicore__ inline void run_diff(GM_ADDR vec_in, GM_ADDR vec_out,
  ...
  if ASCEND_IS_AIV {
    KernelDiff<InputT> op(vec_len, tile_len);
