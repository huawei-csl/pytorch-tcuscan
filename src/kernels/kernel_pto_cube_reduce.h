...
#if defined(__DAV_VEC__) || defined(__DAV_C220__)
...
__aicore__ inline void pto_cube_reduce_aic(
  ...
  // TileLeft maps to AscendC A2 (L0A); TLOAD from an ND GlobalTensor
  // performs the ND->NZ conversion (as CopyGmToL1A + CopyL1ToL0A did).
  using TileA = pto::TileLeft<InputT, S, S>;

  // B-matrix: S×16 all-ones tile (right operand), loaded once.
  // TileRight maps to AscendC B2 (L0B).
  // TEXPANDS fills it with constant 1 (replaces InitConstAllOnesL1 +
  // CopyL1ToL0B).
  ...
  // PTO auto-mode allocates UB/L0 space; no explicit TASSIGN is required.
  TileA tile_a;
  TileB tile_b;
  ...
  // PTO: TEXPANDS on a TileRight (Mat) tile broadcasts scalar 1 into it.
  // Event eb is threaded through TMATMUL/TMATMUL_ACC to enforce ordering.
  auto eb = TEXPANDS(tile_b, InputT(1));

  // ── First tile: TMATMUL (no accumulation) ───────────────────────────────
  // AscendC: CubeIter<false>(ai_core_offset)
  //   -> CopyGmToL1A + CopyL1ToL0A -> TLOAD(tile_a, ga0)
  //   -> cube_unit::Multiply<false>  -> TMATMUL(tile_c0, tile_a, tile_b)
  GTensorA ga0(vec_in + base_offset);
  auto el0 = TLOAD(tile_a, ga0);
  // TMATMUL waits on both the expand event (eb) and the load event (el0).
...
__aicore__ inline void pto_cube_reduce_aiv(__gm__ OutputT* vec_in,
  ...
  //   dst shape: 1 row × 1 col.
  //   The A2A3 implementation accepts both RowMajor ND and ColMajor DN with
  //   Cols==1.  We use RowMajor so the result is directly consumable by TMULS
  //   (which requires RowMajor on both src and dst for A2A3).
  ...
  // Scalar result tile: 1×1, RowMajor (TMULS/TSHRS src and dst requirement).
  ...
  using GTensorIn = pto::GlobalTensor<OutputT, InShape, InStride,
                                      pto::Layout::ND>;
  ...
  using GTensorOut = pto::GlobalTensor<OutputT, OutShape, OutStride,
                                       pto::Layout::ND>;

  // ── WriteOutZero ─────────────────────────────────────────────────────────
  // AscendC: WriteOutZero() -> aiv_id==0 writes scalar 0 to output slot
  //   via copy::CopyScalarToGm (no atomic mode).
  // PTO: TEXPANDS fills a 1×1 tile with 0; TSTORE writes it plain (no AtomicAdd).
  // The zero initialises the output slot so that subsequent AtomicAdd stores
  // accumulate onto a known-zero base.
  if (aiv_id == 0) {
  ...
  auto ec = TCOLSUM(tile_col_sum, tile_part, tile_tmp_col, /*isBinary=*/false,
                    el);
  ...
  if constexpr (std::is_same_v<OutputT, float>) {
    ...
    auto es = TSTORE<TileScalar, GTensorOut,
                     pto::AtomicType::AtomicAdd>(g_out, tile_result, em);
    TSYNC(es);
  } else {
...
//
// Handles optional padding (when vec_len is not aligned to S*S or UB_ALIGNMENT),
// then dispatches pto_cube_reduce_aic on AIC cores and pto_cube_reduce_aiv on
// AIV cores.
//
// Template parameters:
//   InputT - element type of the input vector (half / int8_t)
//   S      - compile-time matmul size; the caller must instantiate for each
//             concrete S value since PTO tile types require compile-time shapes.
// ─────────────────────────────────────────────────────────────────────────────
...
__aicore__ inline void run_pto_cube_reduce(
  ...
  if (vec_len % align_size != 0 || vec_len % UB_ALIGNMENT != 0) {
    ...
    // This is an AscendC kernel; kept unchanged as the PTO spec allows mixing.
  ...
  // ── Dispatch AIV path ────────────────────────────────────────────────────
  // AscendC: if ASCEND_IS_AIV { KernelCompleteCubeReduce<OutputT,true> op; op.Process(); }
  if ASCEND_IS_AIV {
...
#endif  // defined(__DAV_VEC__) || defined(__DAV_C220__)
