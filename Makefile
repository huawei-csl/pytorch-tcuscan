...
profile_fp16_%:
	python3 profile_tcuscan_ops.py --bench $* --dtype fp16

profile_fp32_%:
	python3 profile_tcuscan_ops.py --bench $* --dtype fp32
...
profile_fp16_compress:
	...
	python3 profile_tcuscan_ops.py --bench compress --s 64  --density ${DENSITY} --dtype fp16
	python3 profile_tcuscan_ops.py --bench compress --s 128 --density ${DENSITY} --dtype fp16

profile_fp32_compress:
	python3 profile_tcuscan_ops.py --bench compress --s 32  --density ${DENSITY} --dtype fp32
	python3 profile_tcuscan_ops.py --bench compress --s 64  --density ${DENSITY} --dtype fp32
	python3 profile_tcuscan_ops.py --bench compress --s 128 --density ${DENSITY} --dtype fp32

profile_fp16_seg_scan_sc:
	python3 profile_tcuscan_ops.py --bench seg_scan_sc --dtype fp16 --s 32 --density ${DENSITY} --num_cores 1
	python3 profile_tcuscan_ops.py --bench seg_scan_sc --dtype fp16 --s 64 --density ${DENSITY} --num_cores 1
	python3 profile_tcuscan_ops.py --bench seg_scan_sc --dtype fp16 --s 128 --density ${DENSITY} --num_cores 1

profile_fp16_segmented_sum:
	python3 profile_tcuscan_ops.py --bench segmented_sum --density ${DENSITY} --s 32 --dtype fp16
	python3 profile_tcuscan_ops.py --bench segmented_sum --density ${DENSITY} --s 64 --dtype fp16
	python3 profile_tcuscan_ops.py --bench segmented_sum --density ${DENSITY} --s 128 --dtype fp16

profile_fp32_custom_copy:
	python3 profile_tcuscan_ops.py --bench custom_copy --dtype fp32 --num_cores 1

profile_fp16_custom_copy:
	python3 profile_tcuscan_ops.py --bench custom_copy --dtype fp16 --num_cores 1

profile_fp16_vec_seg_scan:
	python3 profile_tcuscan_ops.py --bench vec_seg_scan_sc --dtype fp16 --density ${DENSITY}  --s 32  --num_cores 1
	python3 profile_tcuscan_ops.py --bench vec_seg_scan_sc --dtype fp16 --density ${DENSITY}  --s 64  --num_cores 1
...
profile_fp16_segmented_scan_real:
	...
	python3 profile_sparse_matrices.py --bench seg_scan_sc --dtype fp16 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --s 64 --num_cores 1
	python3 profile_sparse_matrices.py --bench seg_scan_sc --dtype fp16 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --s 128 --num_cores 1

profile_fp16_vec_segmented_scan_real:
	python3 profile_sparse_matrices.py --bench vec_seg_scan_sc --dtype fp16 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --s 32 --num_cores 1
	python3 profile_sparse_matrices.py --bench vec_seg_scan_sc --dtype fp16 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --s 64 --num_cores 1
	python3 profile_sparse_matrices.py --bench vec_seg_scan_sc --dtype fp16 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --s 128 --num_cores 1

profile_fp16_compress_real:
	python3 profile_sparse_matrices.py --bench compress --s 32  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16
	python3 profile_sparse_matrices.py --bench compress --s 64  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16
	python3 profile_sparse_matrices.py --bench compress --s 128 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16

profile_fp32_compress_real:
	python3 profile_sparse_matrices.py --bench compress --s 32  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp32
	python3 profile_sparse_matrices.py --bench compress --s 64  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp32
	python3 profile_sparse_matrices.py --bench compress --s 128 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp32

profile_mcscan_real:
	python3 profile_sparse_matrices.py --bench mcscan --s 32  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16
	python3 profile_sparse_matrices.py --bench mcscan --s 64  --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16
	python3 profile_sparse_matrices.py --bench mcscan --s 128 --matrixpath ${FULL_SPARSE_MATRIX_PATH} --dtype fp16
...
paper_fig_7:
	python3 profile_tcuscan_ops.py --bench copy --s 128 --dtype fp32
	python3 profile_tcuscan_ops.py --bench mcscan --s 128 --dtype fp16
	python3 profile_tcuscan_ops.py --bench mcscan --s 128 --dtype int8
	python3 profile_tcuscan_ops.py --bench compress --s 128 --density 0.01 --dtype fp32
	python3 profile_tcuscan_ops.py --bench compress --s 128 --density 0.001 --dtype fp32
	python3 profile_tcuscan_ops.py --bench mcscan --s 128 --dtype int8
	python3 profile_tcuscan_ops.py --bench diff_cann --s 128 --dtype fp32
