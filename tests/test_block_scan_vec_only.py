...
def _test_tcuscan_block_scan_vec_only(length: int, dtype: torch.dtype, s: int):
    x = torch.rand(length, device="cpu", dtype=dtype)
    x_npu = x.npu()
    output = tcuscan_ops.run_block_scan_vec_only(x_npu, s)
...
def test_tcuscan_block_scan_vec_only_fp16(length: int):
    _test_tcuscan_block_scan_vec_only(length, torch.float16, 128)
