...
def _test_dtype(vec_len: int, s: int, dtype: torch.dtype):
    ...
    torch.npu.synchronize()
    actual = tcuscan_ops.run_scan_single_cube(x, upper, lower_strict)
    torch.npu.synchronize()
    ...
    ), f"single-cube scan ({dtype}) is wrong. len / s : {vec_len} / {s}. Abs/rel error: {abs_error:.5f} / {rel_error:.7f}"
...
def test_scan_single_cube_s_16(multiplier: int, s: int, dtype: torch.dtype):
...
@pytest.mark.parametrize("dtype", [torch.float16], ids=str)
def test_scan_single_cube_s_32(multiplier: int, s: int, dtype: torch.dtype):
    vec_len = multiplier * NUM_AI_CORES * s * s
    _test_dtype(vec_len, s, dtype)
...
@pytest.mark.parametrize("dtype", [torch.float16], ids=str)
def test_scan_single_cube_s_64(multiplier: int, s: int, dtype: torch.dtype):
    vec_len = multiplier * NUM_AI_CORES * s * s
    _test_dtype(vec_len, s, dtype)
