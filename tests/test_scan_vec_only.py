...
def _test_scan_vec_only(vec_len: int, s: int, dtype: torch.dtype):
    out_dtype = None
    if dtype == torch.float16:
        x = torch.randint(
            -3, 3, size=(vec_len,), dtype=torch.int8, device=NPU_DEVICE
        ).half()
        out_dtype = torch.float32
    else:
        assert False, f"ScanVecOnly supports only fp16. Got {dtype}."

    torch.npu.synchronize()
    x_cpu = x.cpu()
    expected = torch.cumsum(x_cpu, dim=-1, dtype=out_dtype)
    torch.npu.synchronize()
    actual = tcuscan_ops.run_scan_vec_only(x, s)
    torch.npu.synchronize()
