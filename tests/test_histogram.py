...
def _test_tcuscan_histogram(length: int, dtype: torch.dtype, s: int):
    x = torch.rand(length, device="cpu", dtype=dtype)
    x_npu = x.npu()
    num_bins = 20
    output = tcuscan_ops.run_histogram(x_npu, num_bins, s)
...
def test_tcuscan_histogram_fp16(length: int):
    _test_tcuscan_histogram(length, torch.float16, 128)
