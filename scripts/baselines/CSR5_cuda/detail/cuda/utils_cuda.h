...
__inline__ __device__ iT binary_search_right_boundary_kernel(
    const iT *d_row_pointer, const iT key_input, const iT size) {
  iT start = 0;
  iT stop = size - 1;
  iT median;
  iT key_median;
...
template <typename T>
__inline__ __device__ void scan_single(volatile T *s_scan, const int local_id,
                                       const int l) {
  T old_val, new_val;
  if (!local_id) {
    old_val = s_scan[0];
...
// exclusive scan
template <typename T>
__inline__ __device__ void scan_32_plus1(volatile T *s_scan,
                                         const int local_id) {
  int ai, bi;
  const int baseai = 2 * local_id + 1;
  const int basebi = baseai + 1;
...
template <typename T>
__inline__ __device__ T scan_plus1_shfl(volatile T *s_scan, const int local_id,
                                        T r_in, const int seg_num) {
...
template <typename iT>
__forceinline__ __device__ void fetch_x(cudaTextureObject_t d_x_tex, const iT i,
                                        float *x) {
