...
__inline__ __device__ vT candidate(const vT *d_value_partition, const vT *d_x,
                                   cudaTextureObject_t d_x_tex,
                                   const iT *d_column_index_partition,
                                   const iT candidate_index, const vT alpha) {
  vT x = 0;
...
__forceinline__ __device__ vT segmented_sum_shfl(vT tmp_sum,
                                                 const int scansum_offset,
                                                 const int lane_id) {
  vT sum = __shfl_down(tmp_sum, 1);
  sum = lane_id == ANONYMOUSLIB_CSR5_OMEGA - 1 ? 0 : sum;
  vT scan_sum =
      scan_32_shfl(sum);  // scan_32_shfl<vT>(sum, lane_id); // inclusive scan
...
template <typename vT>
__forceinline__ __device__ vT segmented_sum(vT tmp_sum, volatile vT *s_sum,
                                            const int scansum_offset,
                                            const int lane_id) {
  ...
  s_sum[lane_id] +=
      tmp_sum;  // inclusive scan = exclusive scan + original value
...
__inline__ __device__ void partition_fast_track(
    const vT *d_value_partition, const vT *d_x, cudaTextureObject_t d_x_tex,
...
__inline__ __device__ void partition_normal_track(
    const iT *d_column_index_partition, const vT *d_value_partition,
    const vT *d_x, cudaTextureObject_t d_x_tex,
    const uiT *d_partition_descriptor,
    const iT *d_partition_descriptor_offset_pointer,
    const iT *d_partition_descriptor_offset, vT *d_calibrator, vT *d_y,
#if __CUDA_ARCH__ < 300
    volatile vT *s_sum, volatile int *s_scan,
#endif
    const iT par_id, const int lane_id, const int bit_y_offset,
    const int bit_scansum_offset, iT row_start, const bool empty_rows,
    const vT alpha) {
  int start = 0;
  int stop = 0;
  ...
  int offset_pointer =
      empty_rows ? d_partition_descriptor_offset_pointer[par_id] : 0;

  uiT descriptor = d_partition_descriptor[lane_id];

  int y_offset = descriptor >> (32 - bit_y_offset);
  const int scansum_offset =
      (descriptor << bit_y_offset) >> (32 - bit_scansum_offset);
  ...
  sum = candidate<iT, vT>(d_value_partition, d_x, d_x_tex,
                          d_column_index_partition, lane_id, alpha);
...
#pragma unroll
  ...
  if (direct)
    d_y[empty_rows ? d_partition_descriptor_offset[offset_pointer + y_offset]
                   : y_offset] = last_sum;
...
template <typename iT, typename uiT, typename vT, int c_sigma>
__inline__ __device__ void spmv_partition(
    const iT *d_column_index_partition, const vT *d_value_partition,
    const iT *d_row_pointer, const vT *d_x, cudaTextureObject_t d_x_tex,
    const uiT *d_partition_pointer, const uiT *d_partition_descriptor,
    const iT *d_partition_descriptor_offset_pointer,
    const iT *d_partition_descriptor_offset, vT *d_calibrator, vT *d_y,
    const iT par_id, const int lane_id, const int bunch_id,
    const int bit_y_offset, const int bit_scansum_offset, const vT alpha) {
#if __CUDA_ARCH__ < 300
  volatile __shared__ vT
      s_sum[ANONYMOUSLIB_THREAD_GROUP + ANONYMOUSLIB_CSR5_OMEGA / 2];
  volatile __shared__ int
      s_scan[(ANONYMOUSLIB_CSR5_OMEGA + 1) *
             (ANONYMOUSLIB_THREAD_GROUP / ANONYMOUSLIB_CSR5_OMEGA)];
#endif
  ...
  if (row_start == row_stop)  // fast track through reduction
  {
    partition_fast_track<iT, vT, c_sigma>(
        d_value_partition, d_x, d_x_tex, d_column_index_partition, d_calibrator,
#if __CUDA_ARCH__ < 300
    ...
    partition_normal_track<iT, uiT, vT, c_sigma>(
        ...
        d_partition_descriptor_offset, d_calibrator, d_y,
#if __CUDA_ARCH__ < 300
        &s_sum[bunch_id * ANONYMOUSLIB_CSR5_OMEGA],
        &s_scan[bunch_id * (ANONYMOUSLIB_CSR5_OMEGA + 1)],
#endif
        par_id, lane_id, bit_y_offset, bit_scansum_offset, row_start,
        empty_rows, alpha);
  }
}

template <typename iT, typename uiT, typename vT, int c_sigma>
__global__ void spmv_csr5_compute_kernel(
    const iT *d_column_index, const vT *d_value, const iT *d_row_pointer,
    const vT *d_x, cudaTextureObject_t d_x_tex, const uiT *d_partition_pointer,
    const uiT *d_partition_descriptor,
    const iT *d_partition_descriptor_offset_pointer,
    const iT *d_partition_descriptor_offset, vT *d_calibrator, vT *d_y,
    const iT p, const int num_packet, const int bit_y_offset,
    const int bit_scansum_offset, const vT alpha) {
  // warp lane id
  const int lane_id =
  ...
  spmv_partition<iT, uiT, vT, c_sigma>(
      ...
      &d_value[par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma], d_row_pointer, d_x,
      d_x_tex, d_partition_pointer,
      ...
      d_calibrator, d_y, par_id, lane_id, bunch_id, bit_y_offset,
      bit_scansum_offset, alpha);
}

template <typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_calibrate_kernel(const uiT *d_partition_pointer,
                                           const vT *d_calibrator, vT *d_y,
                                           const iT p) {
  ...
  volatile __shared__ vT
      s_sum[ANONYMOUSLIB_THREAD_GROUP / ANONYMOUSLIB_THREAD_BUNCH];

  s_partition_pointer[local_id] =
      global_id < p - 1 ? d_partition_pointer[global_id] & 0x7FFFFFFF : -1;
  s_calibrator[local_id] = sum =
      global_id < p - 1 ? d_calibrator[global_id] : 0;
  __syncthreads();

  // do a fast track if all s_partition_pointer are the same
  if (s_partition_pointer[0] ==
    ...
    if (!bunch_id) {
      sum = lane_id < (ANONYMOUSLIB_THREAD_GROUP / ANONYMOUSLIB_THREAD_BUNCH)
                ? s_sum[lane_id]
                : 0;
      sum = sum_32_shfl<vT>(sum);
    }
...
template <typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_tail_partition_kernel(
    const iT *d_row_pointer, const iT *d_column_index, const vT *d_value,
  ...
  const iT row_start = !blockIdx.x ? (p - 1) * ANONYMOUSLIB_CSR5_OMEGA * sigma
                                   : d_row_pointer[row_id];
  const iT row_stop = d_row_pointer[row_id + 1];

  vT sum = 0;

  for (iT idx = local_id + row_start; idx < row_stop;
       idx += ANONYMOUSLIB_CSR5_OMEGA)
...
#else
  volatile __shared__ vT
      s_sum[ANONYMOUSLIB_CSR5_OMEGA + ANONYMOUSLIB_CSR5_OMEGA / 2];
  s_sum[local_id] = sum;
...
int csr5_spmv(const int sigma, const ANONYMOUSLIB_IT p, const ANONYMOUSLIB_IT m,
              const int bit_y_offset, const int bit_scansum_offset,
              const int num_packet, const ANONYMOUSLIB_IT *row_pointer,
              ...
              const ANONYMOUSLIB_VT alpha, const ANONYMOUSLIB_VT *x,
  ...
  switch (sigma) {
    case 4:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 4><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
      break;
    case 5:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 5><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
    ...
    case 6:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 6><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
    ...
    case 7:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 7><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
    ...
    case 8:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 8><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
    ...
    case 9:
      spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                               ANONYMOUSLIB_VT, 9><<<num_blocks, num_threads>>>(
          column_index, value, row_pointer, x, x_tex, partition_pointer,
          partition_descriptor, partition_descriptor_offset_pointer,
          partition_descriptor_offset, calibrator, y, p, num_packet,
          bit_y_offset, bit_scansum_offset, alpha);
  ...
  spmv_csr5_tail_partition_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                                  ANONYMOUSLIB_VT>
      <<<num_blocks, num_threads>>>(row_pointer, column_index, value, x, x_tex,
                                    y, tail_partition_start, p, sigma, alpha);
