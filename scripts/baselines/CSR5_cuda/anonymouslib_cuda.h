...
class anonymouslibHandle {
 public:
  ...
  int warmup();
  int inputCSR(ANONYMOUSLIB_IT nnz, ANONYMOUSLIB_IT *csr_row_pointer,
               ANONYMOUSLIB_IT *csr_column_index, ANONYMOUSLIB_VT *csr_value);
  int asCSR();
  int asCSR5();
...
template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                       ANONYMOUSLIB_VT>::warmup() {
  format_warmup();
  return 0;
}

template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::
    inputCSR(ANONYMOUSLIB_IT nnz, ANONYMOUSLIB_IT *csr_row_pointer,
             ANONYMOUSLIB_IT *csr_column_index, ANONYMOUSLIB_VT *csr_value) {
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
  ...
  if (_format == ANONYMOUSLIB_FORMAT_CSR5) {
    ...
    err = aosoa_transpose(_csr5_sigma, _nnz, _csr5_partition_pointer,
                          _csr_column_index, _csr_value, false);
    ...
    if (_num_offsets)
      checkCudaErrors(cudaFree(_csr5_partition_descriptor_offset));
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
  ...
  if (_format == ANONYMOUSLIB_FORMAT_CSR) {
    double malloc_time = 0, tile_ptr_time = 0, tile_desc_time = 0,
           transpose_time = 0;
    anonymouslib_timer malloc_timer, tile_ptr_timer, tile_desc_timer,
        transpose_timer;
    // compute sigma
    ...
    if (_bit_y_offset + _bit_scansum_offset >
        sizeof(ANONYMOUSLIB_UIT) * 8 -
            1)  // the 1st bit of bit-flag should be in the first packet
      return ANONYMOUSLIB_UNSUPPORTED_CSR5_OMEGA;

    int bit_all = _bit_y_offset + _bit_scansum_offset + _csr5_sigma;
    _num_packet =
        ceil((double)bit_all / (double)(sizeof(ANONYMOUSLIB_UIT) * 8));
    ...
    checkCudaErrors(cudaMalloc((void **)&_csr5_partition_pointer,
                               (_p + 1) * sizeof(ANONYMOUSLIB_UIT)));

    checkCudaErrors(cudaMalloc(
        (void **)&_csr5_partition_descriptor,
        _p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(ANONYMOUSLIB_UIT)));
    checkCudaErrors(cudaMemset(
        _csr5_partition_descriptor, 0,
        _p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(ANONYMOUSLIB_UIT)));
    ...
    checkCudaErrors(
        cudaMalloc((void **)&_csr5_partition_descriptor_offset_pointer,
                   (_p + 1) * sizeof(ANONYMOUSLIB_IT)));
    checkCudaErrors(cudaMemset(_csr5_partition_descriptor_offset_pointer, 0,
                               (_p + 1) * sizeof(ANONYMOUSLIB_IT)));
    ...
    checkCudaErrors(cudaMemcpy(&tail, &_csr5_partition_pointer[_p - 1],
                               sizeof(ANONYMOUSLIB_UIT),
    ...
    err = generate_partition_descriptor(
    ...
    if (_num_offsets) {
      ...
      malloc_timer.start();
      checkCudaErrors(cudaMalloc((void **)&_csr5_partition_descriptor_offset,
                                 _num_offsets * sizeof(ANONYMOUSLIB_IT)));
      malloc_time += malloc_timer.stop();

      tile_desc_timer.start();
      err = generate_partition_descriptor_offset(
          _csr5_sigma, _p, _bit_y_offset, _bit_scansum_offset, _num_packet,
          _csr_row_pointer, _csr5_partition_pointer, _csr5_partition_descriptor,
    ...
    err = aosoa_transpose(_csr5_sigma, _nnz, _csr5_partition_pointer,
                          _csr_column_index, _csr_value, true);
    if (err != ANONYMOUSLIB_SUCCESS) return ANONYMOUSLIB_CSR_TO_CSR5_FAILED;
    cudaDeviceSynchronize();
...
template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                       ANONYMOUSLIB_VT>::setX(ANONYMOUSLIB_VT *x) {
  int err = ANONYMOUSLIB_SUCCESS;
...
template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                       ANONYMOUSLIB_VT>::spmv(const ANONYMOUSLIB_VT alpha,
                                              ANONYMOUSLIB_VT *y) {
  int err = ANONYMOUSLIB_SUCCESS;
  ...
  if (_format == ANONYMOUSLIB_FORMAT_CSR5) {
    csr5_spmv(_csr5_sigma, _p, _m, _bit_y_offset, _bit_scansum_offset,
              _num_packet, _csr_row_pointer, _csr_column_index, _csr_value,
              _csr5_partition_pointer, _csr5_partition_descriptor,
              _csr5_partition_descriptor_offset_pointer,
              _csr5_partition_descriptor_offset, _temp_calibrator,
              _tail_partition_start, alpha, _x, _x_tex, /*beta,*/ y);
  }
...
template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                       ANONYMOUSLIB_VT>::destroy() {
  cudaDestroyTextureObject(_x_tex);
  return asCSR();
}

template <class ANONYMOUSLIB_IT, class ANONYMOUSLIB_UIT, class ANONYMOUSLIB_VT>
void anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT,
                        ANONYMOUSLIB_VT>::setSigma(int sigma) {
